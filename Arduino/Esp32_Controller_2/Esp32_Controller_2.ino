...
#define CAM_WAKEUP_PIN    26  // Pin untuk bangunkan ESP32-CAM
...
// Function to wake up ESP32-CAM
void wakeupCamera() {
    Serial.println("Waking up ESP32-CAM...");
    digitalWrite(CAM_WAKEUP_PIN, HIGH);
    delay(500);
    digitalWrite(CAM_WAKEUP_PIN, LOW);
    Serial.println("Camera wakeup signal sent");
}

// Function to put ESP32-CAM to sleep
void sleepCamera() {
    Serial.println("Camera sleep command acknowledged");
}
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    Serial.println("Starting ESP32 Barrier Controller with MQTT...");
    ...
    Serial.println("    {\"status\":\"Aktif\"}     -> Wake camera");
    Serial.println("    {\"status\":\"Nonaktif\"} -> Sleep camera");
    Serial.println("========================================");
    ...
    Serial.println("Camera wakeup command sent!");
}
 ...
 * GPIO 32 → Buzzer +
 * GPIO 26 → ESP32-CAM Pin 14 (Wakeup)
 * 5V      → Servo Power
 * GND     → Common Ground
 * 
 * ============================================================================
 * MQTT TOPICS:
 * ============================================================================
 * smarttrain/palang   -> Barrier control
 * smarttrain/camera   -> Camera control
 * 
 * ============================================================================
 * TEST COMMANDS:
 * ============================================================================
 * mosquitto_pub -t "smarttrain/palang" -m '{"status":"Tertutup"}'
 * mosquitto_pub -t "smarttrain/palang" -m '{"status":"Terbuka"}'
 * mosquitto_pub -t "smarttrain/camera" -m '{"status":"Aktif"}'
 * mosquitto_pub -t "smarttrain/camera" -m '{"status":"Nonaktif"}'
 * 
 * ============================================================================
 * FEATURES:
 * ============================================================================
 * - Servo attach/detach otomatis (power save)
 * - Anti-loop protection (debounce 2s + duplicate check 5s)
 * - Buzzer warning sebelum servo bergerak
 * - Camera wakeup trigger via GPIO 26
 * - Same MQTT structure as working ESP32-CAM code
 * 
 * ============================================================================
 */
